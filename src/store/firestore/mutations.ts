...
import { emptySyncedCollection, emptySyncedDoc } from './model';
...
export function mapDocMutations<S>(params: MapMutationsParams): MutationTree<S> {
  ...
  return {
    [`${prefix}.started`](state, mutation) {
      setDeep(state, getStatePath(mutation.context), { ...emptySyncedDoc(), loading: true });
    },
    [`${prefix}.stopped`](state, mutation) {
      setDeep(state, getStatePath(mutation.context), emptySyncedDoc());
    },
    ...
    [`${prefix}.added`](state, mutation) {
      setDeep(state, [...getStatePath(mutation.context), 'item'], mutation.doc);
    },
    [`${prefix}.modified`](state, mutation) {
      setDeep(state, [...getStatePath(mutation.context), 'item'], mutation.doc);
    },
    [`${prefix}.removed`](state, mutation) {
      unsetDeep(state, [...getStatePath(mutation.context), 'item']);
    },
  };
}

export function mapCollectionMutations<S>(params: MapMutationsParams): MutationTree<S> {
  const { prefix, statePath } = params;
  const getStatePath: (context: any) => string[] = typeof statePath === 'function' ? statePath : () => statePath;
