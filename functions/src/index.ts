...
async function updateGlobalStatistics(db: FirebaseFirestore, mutator: StatisticsMutator): Promise<any> {
...
function calculateCarStats(refuels: any[]): any | null {
...
async function updateCarStatistics(db: FirebaseFirestore, carId: string): Promise<any> {
  console.log('updating statistics for car %s', carId);
  const refuelsSnapshot = await db.collection('refuels').where('carId', '==', carId).orderBy('date', 'asc').get();
